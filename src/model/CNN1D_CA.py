...
class CNN_BasicBlock(nn.Module):
    def __init__(self, inplanes, planes, kernel, stride, padding=0, pool=True, pooling_type='avgpool', pool_fix_size=False):
...
class CNN1D(nn.Module):
    def __init__(self, inplanes, max_tree_len, fst_p=8, dropout=0.5):
        super().__init__()
        nb = 3
        # b,1,self.feature_dim*self.max_tree_length
...
class CNN1D_CA(nn.Module):
    def __init__(self, 
        ...
        self.cnn_repre = CNN1D(nc,patch_size,)
        # input 
...
if __name__ == '__main__':
    ...
    patch = 64
    pcnn = CNN1D_CA(2,1,32,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
