...
def get_mimsfcnsevt_twoheadfcn(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MIMSFCN_SEVT(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcn', patch_size=patch_size, slide_window=slide_window)

def get_mimsfcnsevt_twoheadfcnsevt_para1(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MIMSFCN_SEVT(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', structure='parallel1', patch_size=patch_size, slide_window=slide_window)

def get_mimsfcnsevt_twoheadfcnsevt_para2(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MIMSFCN_SEVT(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca',  structure='parallel2', patch_size=patch_size, slide_window=slide_window)

def get_mimsfcnsevt_twoheadfcnsevt_para3(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MIMSFCN_SEVT(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca',  structure='parallel3', patch_size=patch_size, slide_window=slide_window)            

def get_mimsfcnsevt_twoheadfcnsevt_serial1(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MIMSFCN_SEVT(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca',  structure='serial1', patch_size=patch_size, slide_window=slide_window)

def get_mimsfcnsevt_twoheadfcnsevt_serial2(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MIMSFCN_SEVT(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca',  structure='serial2', patch_size=patch_size, slide_window=slide_window)   

def get_mimsfcnsevt_twoheadfcnsevt(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MIMSFCN_SEVT(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', patch_size=patch_size, slide_window=slide_window)

def get_mimsfcnsevt_sharedfcnsevt(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MIMSFCN_SEVT(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='sharedfcnca', patch_size=patch_size, slide_window=slide_window)

def get_mimsfcnsevt_oneheadfcn(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MIMSFCN_SEVT(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='oneheadfcn', patch_size=patch_size, slide_window=slide_window)

class MIMSFCN_SEVT(nn.Module):
    def __init__(self, 
        nclass, 
        nc, 
        globalpool, 
        ...
        self.sa_attn0 = SEVTAttention(model_dim,self.structure)
        # input 
        if self.multiscale == 'twoheadfcnca':
            self.sa_attn1 = SEVTAttention(model_dim,self.structure)
...
if __name__ == '__main__':
    ...
    patch = 64
    #pcnn = get_mimsfcnsevt_twoheadfcn(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
    pcnn = get_mimsfcnsevt_twoheadfcnsevt(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
    #pcnn = get_mimsfcnsevt_sharedfcnca(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
