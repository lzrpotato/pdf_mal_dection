...
logger = logging.getLogger('model.MSFCN_CA')
...
def get_msfcnca_twoheadfcn(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN_CA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcn', patch_size=patch_size, slide_window=slide_window)

def get_msfcnca_twoheadfcnca(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN_CA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', patch_size=patch_size, slide_window=slide_window)

def get_msfcnca_oneheadfcn(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN_CA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='oneheadfcn', patch_size=patch_size, slide_window=slide_window)

class MSFCN_CA(nn.Module):
    def __init__(self, 
        nclass, 
        nc, 
        globalpool, 
        ...
        if self.multiscale == 'twoheadfcn' or self.multiscale == 'twoheadfcnca':
            self.cnn_repre0 = FCN(nc)
            self.cnn_repre1 = FCN(nc)
        else:
            self.cnn_repre0 = FCN(nc)
        ...
        if self.multiscale == 'twoheadfcnca':
            ...
            self.ca_attn1 = nn.Sequential(
                tAttn(in_channels=self.cnn_repre1.out_dim),
                vAttn(in_channels=self.cnn_repre1.out_dim)
            )
        else:
            self.ca_attn0 = nn.Sequential(
                tAttn(in_channels=self.cnn_repre0.out_dim),
        ...
        self.classifier = self.make_classifier(self.cnn_repre1.out_dim+self.cnn_repre1.out_dim,nclass)
...
if __name__ == '__main__':
    ...
    patch = 64
    pcnn = get_msfcnca_twoheadfcn(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
    #pcnn = get_msfcnca_twoheadfcnca(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
