...
logger = logging.getLogger('model.MSFCN1D_CA')
...
def get_msfcn1ca_twoheadfcn(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1D_CA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcn', patch_size=patch_size, slide_window=slide_window)

def get_msfcn1ca_twoheadfcnca(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1D_CA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', patch_size=patch_size, slide_window=slide_window)

def get_msfcn1ca_oneheadfcn(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1D_CA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='oneheadfcn', patch_size=patch_size, slide_window=slide_window)

class MSFCN1D_CA(nn.Module):
    def __init__(self, 
        nclass, 
        nc, 
        globalpool, 
        ...
        self.dowm = nn.Sequential(
                ...
                nn.Conv2d(self.cnn_repre0.out_dim, down_dim, kernel_size=3, stride=3, padding=1),
                nn.ReLU()
            )
        

        self.ca_attn0 = nn.Sequential(
                tAttn(in_channels=down_dim),
                vAttn(in_channels=down_dim)
            )
        # input 
        ...
        self.classifier = self.make_classifier(down_dim + down_dim,nclass)
    ...
    def forward(self, x):
        ...
        for i in x:
            ...
            if self.multiscale == 'twoheadfcn':
                attn_out1 = self.ca_attn0(c_out1)
            elif self.multiscale == 'twoheadfcnca':
                attn_out1 = self.ca_attn1(c_out1)
...
if __name__ == '__main__':
    ...
    pcnn = get_msfcn1ca_twoheadfcnca(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
