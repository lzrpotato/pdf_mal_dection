...
class SCCA(nn.Module):
    def __init__(self, in_dim, stack_num):
        super().__init__()
        self.in_dim = in_dim
        self.stack_num = stack_num
        self.cca_list = nn.ModuleList()
        for i in range(stack_num):
            self.cca_list.append(CrissCrossAttention(in_dim))

    def forward(self, x):
        for cca in self.cca_list:
            x = cca(x)
        return x

def get_msfcn1scca_twoheadfcn(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_SCCA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcn', stack_num=2, patch_size=patch_size, slide_window=slide_window)

def get_msfcn1scca_twoheadfcnscca(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_SCCA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', stack_num=2, patch_size=patch_size, slide_window=slide_window)

def get_msfcn1scca_sharedfcnscca(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_SCCA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='sharedfcnca', stack_num=2, patch_size=patch_size, slide_window=slide_window)

def get_msfcn1scca_oneheadfcn(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_SCCA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='oneheadfcn', stack_num=2, patch_size=patch_size, slide_window=slide_window)

class MSFCN1_SCCA(nn.Module):
    def __init__(self, 
        nclass, 
        nc, 
        globalpool, 
        ...
        self.cca_attn0 = SCCA(model_dim, stack_num)
        # input 
        if self.multiscale == 'twoheadfcnca':
            self.cca_attn1 = SCCA(model_dim, stack_num)
...
if __name__ == '__main__':
    ...
    patch = 64
    #pcnn = get_msfcn1scca_twoheadfcn(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
    pcnn = get_msfcn1scca_twoheadfcnscca(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
    #pcnn = get_msfcn1scca_sharedfcnscca(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
