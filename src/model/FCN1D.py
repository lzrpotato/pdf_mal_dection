...
logger = logging.getLogger('model.FCN1D')
...
class FCN1D(nn.Module):
    def __init__(self, in_channels, scale):
        ...
        #chan_list = [64,128,64]
        if scale > 1:
            self.scalepool = nn.AvgPool1d(scale,scale,padding=scale//2)
        else:
            self.scalepool = None
        self.model = nn.Sequential(
            nn.Conv1d(in_channels=in_channels,out_channels=chan_list[0],kernel_size=8,stride=4,padding=4,bias=True),
            nn.ReLU(),
            
            nn.Conv1d(in_channels=chan_list[0],out_channels=chan_list[1],kernel_size=5,stride=2,padding=2,bias=True),
            nn.ReLU(),

            nn.Conv1d(in_channels=chan_list[1],out_channels=chan_list[2],kernel_size=3,stride=1,padding=1,bias=True),
            nn.ReLU(),
...
def conv1x1_1d(in_planes, out_planes, stride=1, groups=1, dilation=1):
...
class Attention(nn.Module):
    def __init__(self, in_planes):
        super().__init__()
        self.query_w = conv1x1_1d(in_planes, in_planes//8)
        self.key_w = conv1x1_1d(in_planes,in_planes//8)
        self.value_w = conv1x1_1d(in_planes, in_planes//2)
        self.attn_value_w = conv1x1_1d(in_planes//2,in_planes)
        self.softmax = nn.Softmax(2)
        self.sigma = nn.Parameter(torch.zeros(1))

    def forward(self, x):
        query_v = self.query_w(x) # (B,V,T)
        query_v = query_v.permute(0,2,1) # (B,T,V)
        key_v = self.key_w(x)     # (B,V,T)
        value_v = self.value_w(x) # (B,V1,T)
        value_v = value_v.permute(0,2,1) # (B,T,V1)
        logger.debug(f'v_attn query_v {query_v.shape} key_v {key_v.shape}')

        attn = torch.bmm(query_v, key_v)  # (B,T,V) * (B,V,T) = (B,T,T)
        attn = self.softmax(attn)  # (B,T,T)
        attn_value = torch.bmm(attn, value_v)  # (B,T,T) * (B,T,V1) = (B,T,V1)
        attn_value = self.attn_value_w(attn_value.permute(0,2,1))  # (B,V1,T) -> (B,V,T)

        return x + self.sigma * attn_value


class vAttn1d(nn.Module):
    def __init__(self, in_planes):
        super().__init__()
        self.query_w = conv1x1_1d(in_planes, in_planes//8)
        self.key_w = conv1x1_1d(in_planes,in_planes//8)
    ...
    def forward(self, x):
        query_v = self.query_w(x) # (B,T,V)
        query_v = query_v.permute(0,2,1) # (B,V,T)
        key_v = self.key_w(x)     # (B,T,V)
        value_v = self.value_w(x) # (B,T1,V)
        value_v = value_v.permute(0,2,1) # (B,V,T1)
        logger.debug(f'v_attn query_v {query_v.shape} key_v {key_v.shape}')

        attn = torch.bmm(query_v, key_v)  # (B,V,T) * (B,T,V) = (B,V,V)
        attn = self.softmax(attn)  # (B,V,V)
        attn_value = torch.bmm(attn, value_v)  # (B,V,V) * (B,V,T1) = (B,V,T1)
        attn_value = self.attn_value_w(attn_value.permute(0,2,1))  # (B,T1,V) -> (B,T,V)

        return x + self.sigma * attn_value

class tAttn1d(nn.Module):
    def __init__(self, in_planes):
        super().__init__()
        self.query_w = conv1x1_1d(in_planes, in_planes//8)
        self.key_w = conv1x1_1d(in_planes,in_planes//8)
...
def get_fcn1d(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1D(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='oneheadfcn', attention=0, patch_size=patch_size, slide_window=slide_window)

def get_fcn1d_attn(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1D(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='oneheadfcn', attention=1, patch_size=patch_size, slide_window=slide_window)

def get_fcn1d_ca1d(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1D(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='oneheadfcn', attention=2, patch_size=patch_size, slide_window=slide_window)

class MSFCN1D(nn.Module):
    def __init__(self, 
        nclass, 
        nc, 
        globalpool, 
        ...
        if self.multiscale == 'twoheadfcn' or self.multiscale == 'twoheadfcnca':
            self.cnn_repre0 = FCN1D(in_channels,1)
            self.cnn_repre1 = FCN1D(in_channels,2)
        else:
            self.cnn_repre0 = FCN1D(in_channels, 1)
        
        if self.attention == 1:
            self.attn = Attention(self.cnn_repre0.out_dim)
        ...
        if self.multiscale == 'oneheadfcn':
            mlp_dim = self.cnn_repre0.out_dim
        else:
            pass

        self.classifier = self.make_classifier(mlp_dim,nclass)
    ...
    def forward(self, x):
        ...
        for i in x:
            ...
            c_in_0 = c_in_0.reshape(1,h*w, nw)
            ...
            logger.debug(f'c_out0 {c_out0.shape}')
            ...
            if self.attention == True:
                c_out0 = self.attn(c_out0)

            out0 = self.global_pool(c_out0)
            out0 = self.flatten(out0)
            logger.debug(f'globalpool {out0.shape}')

            if self.multiscale != 'oneheadfcn':
                out1 = self.global_pool(c_out1)
...
if __name__ == '__main__':
    ...
    patch = 64
    pcnn = get_fcn1d_attn(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
    #pcnn = get_fcn1d(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
