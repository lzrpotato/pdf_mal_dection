...
def get_fxmsfcn1cca_twoheadfcn(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return FXMSFCN1_CCA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcn', patch_size=patch_size, slide_window=slide_window)

def get_fxmsfcn1cca_twoheadfcnca(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return FXMSFCN1_CCA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', patch_size=patch_size, slide_window=slide_window)

def get_fxmsfcn1cca_sharedfcnca(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return FXMSFCN1_CCA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='sharedfcnca', patch_size=patch_size, slide_window=slide_window)

def get_fxmsfcn1cca_oneheadfcn(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return FXMSFCN1_CCA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='oneheadfcn', patch_size=patch_size, slide_window=slide_window)

class FXMSFCN1_CCA(nn.Module):
    def __init__(self, 
        nclass, 
        nc, 
        globalpool, 
    ...
    def _pad_to_max(self, seq, max_length):
        """
        pad pdf file to fit max length
        """
        batch, c, seq_len = seq.size()
        need = max_length - seq_len
        if need > 0:
            seq = F.pad(seq,(0,need))
        elif need < 0:
    ...
    def _view_slide_window(self, seq: torch.Tensor, max_length):
        """
        change the pdf file to sliding window view
        """
        
        if max_length != -1:
            seq = self._pad_to_max(seq, max_length)
        else:
            seq = self._pad_to_grid(seq)
        batch, c, seq_len = seq.size()
        h,w = self.patch_size
...
if __name__ == '__main__':
    ...
    i = torch.ones((8,4248001))
    from torch.profiler import profile, ProfilerActivity, record_function

    patch = 64
    #pcnn = get_fxmsfcn1cca_twoheadfcn(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
    pcnn = get_fxmsfcn1cca_twoheadfcnca(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
    #pcnn = get_fxmsfcn1cca_sharedfcnca(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
