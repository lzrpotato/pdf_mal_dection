...
from src.module.SerialAttention import SerialAttention
...
def get_mimsfcnsa_twoheadfcn(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MIMSFCN_SA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcn', patch_size=patch_size, slide_window=slide_window)

def get_mimsfcnsa_twoheadfcnsa(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MIMSFCN_SA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', patch_size=patch_size, slide_window=slide_window)

def get_mimsfcnsa_sharedfcnsa(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MIMSFCN_SA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='sharedfcnca', patch_size=patch_size, slide_window=slide_window)

def get_mimsfcnsa_oneheadfcn(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MIMSFCN_SA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='oneheadfcn', patch_size=patch_size, slide_window=slide_window)

class MIMSFCN_SA(nn.Module):
    def __init__(self, 
        nclass, 
        nc, 
        globalpool, 
        ...
        self.sa_attn0 = SerialAttention(model_dim)
        # input 
        if self.multiscale == 'twoheadfcnca':
            self.sa_attn1 = SerialAttention(model_dim)
...
if __name__ == '__main__':
    ...
    patch = 64
    #pcnn = get_mimsfcnsa_twoheadfcn(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
    pcnn = get_mimsfcnsa_twoheadfcnsa(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
    #pcnn = get_mimsfcnsa_sharedfcnca(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
