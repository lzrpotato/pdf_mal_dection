...
logger = logging.getLogger('model.MSFCN1_CCCA')
...
class ChannelCrissCrossAttention(nn.Module):
    ...
    def __init__(self, in_dim):
        ...
        self.c_reconv = nn.Conv2d(in_channels=in_dim//8, out_channels=in_dim, kernel_size=1)
        self.v_reconv = nn.Conv2d(in_channels=in_dim//8, out_channels=in_dim, kernel_size=1)
        self.t_reconv = nn.Conv2d(in_channels=in_dim//8, out_channels=in_dim, kernel_size=1)
        self.INF = INF
    ...
    def forward(self, x):
        ...
        # (B,C,V,T) -> (B,V,T,C,C)
        proj_query_C = proj_query.view(B*T*V,C)
        logger.debug('proj_query_C {} {}'.format(proj_query_C.view(-1,C,1).shape,proj_query_C.view(-1,1,C).shape))
        proj_query_CC = torch.bmm(proj_query_C.view(-1,C,1), proj_query_C.view(-1,1,C))
        # (B,C,V,T) -> (B,C,T,V,V)
        proj_query_V = proj_query.view(B*C*T,V)
        proj_query_VV = torch.bmm(proj_query_V.view(-1,V,1), proj_query_V.view(-1,1,V))
        # (B,C,V,T) -> (B,C,V,T,T)
        proj_query_T = proj_query.view(B*C*V,T)
        proj_query_TT = torch.bmm(proj_query_T.view(-1,T,1).contiguous(), proj_query_T.view(-1,1,T).contiguous())
        
        # (B,C,V,T)->(B,C1,V,T)
        proj_key = self.key_conv(x)  
        # (B,C,V,T) -> (B,V,T,C,C)
        proj_key_C = proj_key.view(B*T*V,C)
        proj_key_CC = torch.bmm(proj_key_C.view(-1,C,1), proj_key_C.view(-1,1,C))
        # (B,C,V,T) -> (B,C,T,V,V)
        proj_key_V = proj_key.view(B*C*T,V)
        proj_key_VV = torch.bmm(proj_key_V.view(-1,V,1), proj_key_V.view(-1,1,V))
        # (B,C,V,T) -> (B,C,V,T,T)
        proj_key_T = proj_key.view(B*C*V,T)
        proj_key_TT = torch.bmm(proj_key_T.view(-1,T,1), proj_key_T.view(-1,1,T))
        ...
        # (B*T*V,C,C) x (B*T*V,C,C) -> (B*T*V,C,C) -> (B,C,V,T,C)
        energy_CC = torch.bmm(proj_query_CC, proj_key_CC).view(B,T,V,C,C).permute(0,3,2,1,4)
        # (B*C*T,V,V) x (B*C*T,V,V) -> (B*C*T,V,V) -> (B,C,V,T,V)
        energy_VV = torch.bmm(proj_query_VV, proj_key_VV).view(B,C,T,V,V).permute(0,1,3,2,4)
        # (B*C*V,T,T) x (B*C*V,T,T) -> (B*C*V,T,T) -> (B,C,V,T,T)
        energy_TT = torch.bmm(proj_query_TT, proj_key_TT).view(B,C,V,T,T)
...
def get_msfcn1ccca_twoheadfcn(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_CCCA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcn', patch_size=patch_size, slide_window=slide_window)

def get_msfcn1ccca_twoheadfcnccca(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_CCCA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', patch_size=patch_size, slide_window=slide_window)

def get_msfcn1ccca_sharedfcnccca(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_CCCA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='sharedfcnca', patch_size=patch_size, slide_window=slide_window)

def get_msfcn1ccca_oneheadfcn(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_CCCA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='oneheadfcn', patch_size=patch_size, slide_window=slide_window)

class MSFCN1_CCCA(nn.Module):
    def __init__(self, 
        nclass, 
        nc, 
        globalpool, 
...
if __name__ == '__main__':
    ...
    pcnn = get_msfcn1ccca_twoheadfcnccca(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
