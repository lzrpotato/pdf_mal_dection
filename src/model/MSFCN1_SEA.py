...
def get_msfcn1sea_twoheadfcn(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_SEA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcn', patch_size=patch_size, slide_window=slide_window)

def get_msfcn1sea_twoheadfcnsea(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_SEA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', patch_size=patch_size, slide_window=slide_window)

def get_msfcn1sea_twoheadfcnsea_para(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_SEA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', structure='parallel', patch_size=patch_size, slide_window=slide_window)

def get_msfcn1sea_twoheadfcnsea_para1(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_SEA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', structure='parallel1', patch_size=patch_size, slide_window=slide_window)

def get_msfcn1sea_twoheadfcnsea_para2(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_SEA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', structure='parallel2', patch_size=patch_size, slide_window=slide_window)

def get_msfcn1sea_twoheadfcnsea_serial1(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_SEA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', structure='serial1', patch_size=patch_size, slide_window=slide_window)

def get_msfcn1sea_twoheadfcnsea_serial2(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_SEA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', structure='serial2', patch_size=patch_size, slide_window=slide_window)

def get_msfcn1sea_twoheadfcnsea_para_r(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_SEA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', structure='parallel', sea_relu=True, patch_size=patch_size, slide_window=slide_window)

def get_msfcn1sea_twoheadfcnsea_para1_r(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_SEA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', structure='parallel1', sea_relu=True, patch_size=patch_size, slide_window=slide_window)

def get_msfcn1sea_twoheadfcnsea_para2_r(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_SEA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', structure='parallel2', sea_relu=True, patch_size=patch_size, slide_window=slide_window)

def get_msfcn1sea_twoheadfcnsea_serial1_r(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_SEA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', structure='serial1', sea_relu=True, patch_size=patch_size, slide_window=slide_window)

def get_msfcn1sea_twoheadfcnsea_serial2_r(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_SEA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='twoheadfcnca', structure='serial2', sea_relu=True, patch_size=patch_size, slide_window=slide_window)

def get_msfcn1sea_sharedfcnsea(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_SEA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='sharedfcnca', patch_size=patch_size, slide_window=slide_window)

def get_msfcn1sea_oneheadfcn(nclass=2, inplane=1, globalpool='maxpool', num_layers=1, patch_size=(256,256), slide_window=(256*256,256*256/2)):
    return MSFCN1_SEA(nclass, nc=inplane, globalpool=globalpool, num_layers=num_layers,
            multiscale='oneheadfcn', patch_size=patch_size, slide_window=slide_window)

class MSFCN1_SEA(nn.Module):
    def __init__(self, 
        nclass, 
        nc, 
        globalpool, 
        ...
        self.attn0 = SEAttention(model_dim,self.structure, self.sea_relu)
        # input 
        if self.multiscale == 'twoheadfcnca':
            self.attn1 = SEAttention(model_dim,self.structure, self.sea_relu)
...
if __name__ == '__main__':
    ...
    patch = 64
    #pcnn = get_msfcn1sea_twoheadfcn(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
    pcnn = get_msfcn1sea_twoheadfcnsea(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
    #pcnn = get_msfcn1sea_sharedfcnsea(2,1,'maxpool',num_layers=1,patch_size=(patch,patch),slide_window=(patch*patch,patch*patch/2))
