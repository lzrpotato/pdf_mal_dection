...
from src.model.CNN_DANet import CNN_DAttn
...
logger = logging.getLogger('trainer.CNNDAttnTrainer')
...
def train(args):
    ...
    # setup database
    db_name = f'exp_cnn_dattn_{dnn_mode}_1.db'
    db = Database(db_name)
    ...
    for i in range(dl.nfold):
        if args.fold != i:
            continue
        exp_keys = {'exp':args.exp, 'nclass': args.nclass, 'dnn': f'CNNDAttn_{dnn_mode}', 'dataset':args.dataset, 'stride': args.stride ,'fold':i}
        if db.check_finished(exp_keys):
            continue
        model = CNNDAttnTrainer(args, dl.class_to_index)
...
class CNNDAttnTrainer(BaseTrainer):
    def __init__(self,args,class_to_index):
        ...
        if self.args.slide_window:
            ...
            slide_window = None
        self.model = CNN_DAttn(self.args.nclass,self.args.nc,
    ...
    def forward(self, batch):
        ...
        logger.debug('cnnDAttn {} {}'.format(X[0].shape, y[0].shape))
        y_hat = self.model(X)
        return y_hat
