...
logger = logging.getLogger(f'trainer.{dnn_id}')
...
def train(args):
    args.determ = True
    dl = TSDataLoader(args.dataset, batch_size=args.batch_size, nfold=args.nfold, embedding=True)
    dl.setup()
    ...
    # setup database
    db_name = f'exp_{dnn_id}_{dnn_mode}_1.db'
    db = Database(db_name)

    dnn_model = f'{dnn_id}_{dnn_mode}'

    resuls = []
    for i in range(dl.nfold):
        if args.fold != i:
            continue
        exp_keys = {'exp':args.exp, 'nclass': args.nclass, 'dnn': dnn_model, 'dataset':args.dataset, 'stride': args.stride ,'fold':i}
        if db.check_finished(exp_keys):
            continue
        model = CNNRFTrainer(args, dl.class_to_index)
...
def setup_arg():
    ...
    parser.add_argument('--stride', type=float, default=0, help='Stride, 1/2 or 3/4 of the window size')
...
class CNNRFTrainer(BaseTrainer):
    def __init__(self,args,class_to_index):
        super().__init__()
        self.args = args
        self.class_to_index = class_to_index

        self.model = ModelC(self.args.nclass)
