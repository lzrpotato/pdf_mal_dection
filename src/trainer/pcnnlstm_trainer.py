...
logger = logging.getLogger('trainer.PCNNLSTMTrainer')
...
def train(args):
    ...
    # setup database
    db_name = f'exp_pcnn_lstm_{dnn_mode}_1.db'
    db = Database(db_name)
    ...
    for i in range(dl.nfold):
        if args.fold != i:
            continue
        exp_keys = {'exp':args.exp, 'nclass': args.nclass, 'dnn': f'PCNN_{"fz" if args.freeze else "nfz"}_{args.cnn_model_name}_LSTM_{dnn_mode}', 'dataset':args.dataset, 'stride': args.stride ,'fold':i}
        if db.check_finished(exp_keys):
            continue
        model = PCNNLSTMTrainer(args, dl.class_to_index)
...
class PCNNLSTMTrainer(BaseTrainer):
    def __init__(self,args,class_to_index):
        ...
        if self.args.slide_window:
            ...
            slide_window = None
        self.model = PCNN_LSTM(self.args.nclass,self.args.nc,self.args.cnn_model_name,self.args.freeze,self.args.hidden,self.args.bidirection,
    ...
    def forward(self, batch):
        ...
        logger.debug('pcnnlstm {} {}'.format(X[0].shape, y[0].shape))
        y_hat = self.model(X)
        return y_hat
