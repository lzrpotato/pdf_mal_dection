...
from src.dataloader.image_dataloader import ImageDataLoader
from src.model import Resnet101, MobileNetV3, VGG19, SqueezeNet11
...
logger = logging.getLogger('trainer.cnntrainer')

def param_setting(args):
    return f"""ep={args.epochs}_h={args.height}_w={args.width}_nl={args.num_layers}_pt={args.patience}"""

def unique_name(exp_keys):
    return f"""exp={exp_keys['exp']}_nclass={exp_keys['nclass']}_dnn={exp_keys['dnn']}_ds={exp_keys['dataset']}_fold={exp_keys['fold']}"""
...
def train(args):
    args.determ = True
    dl = ImageDataLoader(args.dataset, batch_size=args.batch_size, nfold=args.nfold)
    dl.setup()
    ...
    for i in range(dl.nfold):
        if args.fold != i:
            continue
        exp_keys = {'exp':args.exp, 'nclass': args.nclass, 'dnn': args.model, 'dataset':args.dataset, 'stride': 'n/a', 'fold':i}
        if db.check_finished(exp_keys):
            continue
        model = CNNTrainer(args, dl.class_to_index)
...
def setup_arg():
    ...
    parser.add_argument('--dataset', type=str, default='byte', choices=['byte','markov'])
    parser.add_argument('--model', type=str, default='vgg19', choices=['resnet101','vgg19','squeezenet11','mobilenetv3'])
    parser.add_argument('--pretrained', action='store_true', default=False, help='Using pretrained weights')
...
class CNNTrainer(BaseTrainer):
    def __init__(self,args,class_to_index):
        ...
        elif args.model == 'squeezenet11':
            self.model = SqueezeNet11(args)
        elif args.model == 'mobilenetv3':
            self.model = MobileNetV3(args)

        # choose the weight-initialization mode
        if self.args.pretrained:
            self.model.pretrained_weights()
        else:
            self.model.random_weights()
        
        # modify model to fit the dataset
    ...
    def forward(self, batch):
        ...
        logger.debug('cnn {} {}'.format(X[0].shape, y[0].shape))
        y_hat = self.model(X)
        return y_hat
