...
from src.model.FCN_TRAN import FCN_TRAN
...
logger = logging.getLogger('trainer.FCNTRANTrainer')
...
def train(args):
    ...
    # setup database
    db_name = f'exp_fcn_tran_{dnn_mode}_1.db'
    db = Database(db_name)

    dnn_model = f'FCNTRAN_{dnn_mode}'
    ...
    for i in range(dl.nfold):
        ...
        if db.check_finished(exp_keys):
            continue
        model = FCNTRANTrainer(args, dl.class_to_index)
...
class FCNTRANTrainer(BaseTrainer):
    def __init__(self,args,class_to_index):
        ...
        if self.args.slide_window:
            ...
            slide_window = None
        self.model = FCN_TRAN(self.args.nclass,self.args.nc,
