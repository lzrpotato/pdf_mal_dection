...
from src.model.PCNN_Attention import PCNN_Attention
...
logger = logging.getLogger('trainer.PCNNAttnTrainer')
...
def train(args):
    ...
    # setup database
    db_name = f'exp_pcnn_attn_{dnn_mode}_1.db'
    db = Database(db_name)
    ...
    for i in range(dl.nfold):
        if args.fold != i:
            continue
        exp_keys = {'exp':args.exp, 'nclass': args.nclass, 'dnn': f'PCNNAttn_{"fz" if args.freeze else "nfz"}_{args.cnn_model_name}_{dnn_mode}', 'dataset':args.dataset, 'stride': args.stride ,'fold':i}
        if db.check_finished(exp_keys):
            continue
        model = PCNNAttnTrainer(args, dl.class_to_index)
...
def setup_arg():
    ...
    parser.add_argument('--cnn-model-name', type=str, default='mobilenetv3', choices=['resnet101','vgg19','squeezenet11','mobilenetv3'])
    parser.add_argument('--freeze', action='store_true', default=False, help='freeze cnn part to reduct train time')
...
class PCNNAttnTrainer(BaseTrainer):
    def __init__(self,args,class_to_index):
        ...
        if self.args.slide_window:
            ...
            slide_window = None
        self.model = PCNN_Attention(self.args.nclass,self.args.nc,self.args.cnn_model_name,self.args.freeze,self.args.bidirection,
    ...
    def forward(self, batch):
        ...
        logger.debug('pcnnAttn {} {}'.format(X[0].shape, y[0].shape))
        y_hat = self.model(X)
        return y_hat
