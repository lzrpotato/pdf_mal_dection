...
from src.model.CNN_TRAN import CNN_TRAN
...
logger = logging.getLogger('trainer.CNNTRANTrainer')
...
def train(args):
    ...
    # setup database
    db_name = f'exp_cnn_tran_{dnn_mode}_1.db'
    db = Database(db_name)

    dnn_model = f'CNNTRAN_{dnn_mode}'
    ...
    for i in range(dl.nfold):
        ...
        if db.check_finished(exp_keys):
            continue
        model = CNNTRANTrainer(args, dl.class_to_index)
...
class CNNTRANTrainer(BaseTrainer):
    def __init__(self,args,class_to_index):
        ...
        if self.args.slide_window:
            ...
            slide_window = None
        self.model = CNN_TRAN(self.args.nclass,self.args.nc,
                        batch_size=self.args.batch_size, pool=self.args.pool, num_layers=self.args.num_layers,
