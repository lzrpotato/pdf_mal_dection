...
from src.model.CNN_CA import CNN_CA
...
logger = logging.getLogger('trainer.CNNCATrainer')
...
def train(args):
    ...
    # setup database
    db_name = f'exp_CNN_CA_{dnn_mode}_1.db'
    db = Database(db_name)
    dnn_name = f'CNNCA_{dnn_mode}'
    ...
    for i in range(dl.nfold):
        ...
        if db.check_finished(exp_keys):
            continue
        model = CNNCATrainer(args, dl.class_to_index)
...
class CNNCATrainer(BaseTrainer):
    def __init__(self,args,class_to_index):
        ...
        if self.args.slide_window:
            ...
            slide_window = None
        self.model = CNN_CA(self.args.nclass,self.args.nc,
