...
from src.model.RESNET_CA import Resnet_CA
...
def train(args):
    ...
    # setup database
    db_name = f'exp_{args.dnn}_{dnn_mode}_1.db'
    db = Database(db_name)
    dnn_name = f'{args.dnn}_{dnn_mode}'
...
def setup_arg():
    ...
    parser.add_argument('--dnn', type=str, choices=['Resnet_CA'])
...
class MainTrainer(BaseTrainer):
    def __init__(self,args,class_to_index):
        ...
        if args.dnn == 'Resnet_CA':
            self.model = Resnet_CA(self.args.nclass,self.args.nc,
