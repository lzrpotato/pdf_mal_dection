...
from src.embedding.word2vec import NGramLanguageModeler
...
def train(args):
    ...
    for i in range(dl.nfold):
        model = NGramLanguageModelerTrainer(args)
...
class NGramLanguageModelerTrainer(EmbeddingBase):
    def __init__(self,args):
        super().__init__()
        self.args = args
        self.model = NGramLanguageModeler(self.args.vocab_size, self.args.context_size, self.args.embedding_dim)
